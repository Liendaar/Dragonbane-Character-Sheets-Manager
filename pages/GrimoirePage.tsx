...
const SpellCard: React.FC<{ 
    ...
    return (
        <div className="bg-[#2a2a2a]/70 border-2 border-[#404040] rounded-lg p-4 shadow-md hover:shadow-lg hover:border-[#2D7A73] transition-all">
            <div className="flex justify-between items-start mb-2">
                <div className="flex-1">
                    <div className="flex items-center space-x-2 mb-1">
                        <h3 className="font-title text-lg font-bold text-[#4ade80]">{spell.nom}</h3>
                    ...
                    <p className="text-xs text-gray-400 italic">{spell.ecole}</p>
                </div>
            ...
            <div className="space-y-1 text-sm mb-2 text-gray-300">
                {spell.condition_prealable && (
                    <div><span className="font-bold text-gray-200">Prérequis:</span> {spell.condition_prealable}</div>
                )}
                {spell.prerequis && (
                    <div><span className="font-bold text-gray-200">Composantes:</span> {spell.prerequis}</div>
                )}
                {spell.temps_incantation && (
                    <div><span className="font-bold text-gray-200">Temps d'incantation:</span> {spell.temps_incantation}</div>
                )}
                {spell.portee && (
                    <div><span className="font-bold text-gray-200">Portée:</span> {spell.portee}</div>
                )}
                {spell.duree && (
                    <div><span className="font-bold text-gray-200">Durée:</span> {spell.duree}</div>
                )}
            </div>
            
            <p className="text-sm text-gray-300 border-t border-gray-600 pt-2">
                {spell.description}
            </p>
            
            {isCantrip && (
                <div className="mt-2 text-xs text-[#4ade80] italic">
        ...
        <div className="p-2 md:p-4 bg-[#1a1a1a] min-h-screen">
            <div className="max-w-6xl mx-auto space-y-4">
                <header className="flex justify-between items-center mb-4">
                    ...
                    <h1 className="text-3xl font-title font-bold text-[#4ade80]">Grimoire de {character.name || 'Personnage'}</h1>
                </header>

                {/* Compteur de sorts mémorisés */}
                <div className={`bg-[#2a2a2a]/70 border-2 ${isOverLimit ? 'border-red-500' : 'border-[#2D7A73]'} rounded-lg p-4 text-center`}>
                    <div className="text-sm text-gray-400">Sorts mémorisés (hors tours de magie)</div>
                    <div className={`text-3xl font-bold ${isOverLimit ? 'text-red-500' : 'text-[#4ade80]'}`}>
                ...
                <button
                    ...
                    className="w-full bg-[#2D7A73] hover:bg-[#3d9a8a] text-white font-bold py-2 px-4 rounded transition-colors"
                >
                ...
                {/* Sélecteur de sorts */}
                {showSpellSelector && (
                    <div className="bg-[#2a2a2a]/70 border-2 border-[#2D7A73] rounded-lg p-4">
                        <h2 className="font-title text-xl font-bold text-[#4ade80] mb-4">Sélectionner un sort</h2>
                        
                        {/* Sélecteur d'école */}
                        <select 
                            value={selectedSchool} 
                            onChange={(e) => setSelectedSchool(e.target.value)}
                            className="w-full p-2 border border-gray-600 rounded mb-4 bg-[#1a1a1a] text-gray-200"
                        >
                                ...
                                {availableSpells
                                    ...
                                    .map((spell, i) => (
                                        <div 
                                            ...
                                            className="p-3 border border-gray-600 rounded hover:bg-[#3a3a3a] cursor-pointer"
                                        >
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    <div className="font-bold text-gray-200">{spell.nom}</div>
                                                    <div className="text-xs text-gray-400">
                                                        {spell.rang === 'Tour de magie' ? 'Tour de magie' : `Rang ${spell.rang}`} - {spell.ecole}
                                                    </div>
                                                </div>
                                                <button className="text-[#4ade80] text-sm">Ajouter</button>
                                            </div>
                                            <p className="text-sm text-gray-300 mt-1">{spell.description}</p>
                                        </div>
                                    ))}
                            </div>
                        )}
                ...
                <div>
                    {character.grimoire.length === 0 ? (
                        <div className="text-center text-gray-400 py-10">
                            <h2 className="font-title text-2xl font-bold text-[#4ade80] mb-4">Sorts connus (0)</h2>
                        ...
                        <>
                            ...
                            {(() => {
                                ...
                                return (
                                    <div className="mb-8">
                                        <h2 className="font-title text-2xl font-bold text-[#4ade80] mb-4 border-b-2 border-[#2D7A73] pb-2">
                                ...
                                return (
                                    <div className="space-y-8">
                                        {Object.entries(spellsBySchool).sort(([a], [b]) => a.localeCompare(b)).map(([school, spells]) => (
                                            <div key={school} className="border-2 border-[#2D7A73] rounded-lg p-4 bg-[#2a2a2a]/50">
                                                <h2 className="font-title text-2xl font-bold text-[#4ade80] mb-4">
